    ...
    Object.entries(datos).forEach(([k, v]) => {
        ...
        if (k == 'albumes') {
            if (v.length != 0) {
                ...
                albumes = albumes.substring(0, albumes.length - 2);
                $("#mas-info").append('<p class="text-start mt-4"><u>Popular albums</u>: <em>' + albumes + '</em></p>');
                $("#mas-info").append('<button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-albums-info" aria-expanded="false" aria-controls="collapse-info">Show more about these albums</button>');
                $("#mas-info").append('<div class="collapse" id="collapse-albums-info"><div id="albums-info" class="card card-body mt-3"></div></div>');
            }
        }

        if (k.includes('cancionAlbum')) {
            if (v.length != 0) {
                $("#mas-info-card").removeClass("d-none");
                if (listaAlbumes.includes(v['album'])) {
                    $('#albums-info').append('<p class="text-start mb-4"><i class="bi bi-arrow-right-circle mx-3"></i> ' + v['cancion'] + '</p>');

                } else {
                    listaAlbumes.push(v['album']);
                    $('#albums-info').append('<h3 class="mb-3"><em>' + v['album'] + '</em></h3><p class="text-start mb-4"><i class="bi bi-arrow-right-circle mx-3"></i> ' + v['cancion'] + '</p>');
                }
            }
        }
    });
